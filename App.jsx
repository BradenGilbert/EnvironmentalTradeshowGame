...
const stageSteps = [
  { id: "scenario", label: "Bucket A" },
  { id: "tools", label: "Bucket B" },
...
function StageTracker({ stage, scenario, selectedTool, modifier }) {
  ...
  const progressPercent =
    activeIndex >= 0 ? ((activeIndex + 1) / stageSteps.length) * 100 : 0;

  return (
    <div className="stage-progress-shell">
      <div className="stage-progress-header">
        <div>
          <div className="panel-label">Round Progress</div>
          <div className="stage-progress-title">Incident Flow Tracker</div>
        </div>
        <div className="stage-progress-count">
          {activeIndex + 1 < 1 ? 0 : activeIndex + 1} / {stageSteps.length}
        </div>
      </div>
      ...
      <div className="stage-step-row">
        {stageSteps.map((step, index) => {
          const isComplete = activeIndex > index;
          const isActive = activeIndex === index;
          const stateClass = isActive ? "stage-dot-active" : isComplete ? "stage-dot-complete" : "stage-dot-idle";

          return (
            <div key={step.id} className="stage-step-item">
              <div className={`stage-dot ${stateClass}`}>{index + 1}</div>
...
function AdjustmentTicker({ feed }) {
  if (!feed.length) {
    return (
      <div className="ticker-inline-empty">
        No bonuses or penalties applied yet.
      </div>
    );
  }

  return (
    <div className="ticker-inline-list">
      {feed.slice(0, 6).map((item) => (
        <div
          key={item.instanceId}
          className={`ticker-inline-item ${item.type === "penalty" ? "ticker-inline-penalty" : "ticker-inline-bonus"}`}
        >
          {item.type === "penalty" ? "Penalty:" : "Bonus:"} {item.label}
        </div>
      ))}
    </div>
  );
        ...
        <div className="topbar">
          <div>
            <div className="title-row">
              ...
              <div className="title-row-name">The Compliance Gauntlet</div>
            </div>
          ...
          <div>
            ...
            {stage === "response" && scenario && selectedTool && modifier && (
              <div className="page-stack">
                ...
                <Card>
                  <div className="inline-feed-panel">
                    <div className="panel-label">Live Adjustments Feed</div>
